...
nodes_counter = 0  # number of nodes expanded
...
seen = []
# 'node'
# ++++++++++++++++++++++++++++++For nxn puzzle we need to change line 99,195,338-360 I made the comments.+++++++++++++++++++++++++++++++++++++++++++++++++++
...
def Manhattan_Distance_and_misplaced(problem, choice):
    ...
    real_index_c = 0
    if choice == 3:
        # number 1-9 for nxn replace 9 with n+1
        for l in range(1, 9):

            for q in range(0, 3):  # For nxn replace 3 with n
                for w in range(0, 3):  # For nxn replace 3 with n
    ...
    if choice == 2:
        # matrix
        for z in range(0, 3):  # nxn replace 3 with n
            for x in range(0, 3):
...
if __name__ == '__main__':
    ...
    if inputc == 2:  # https://www.geeksforgeeks.org/take-matrix-input-from-user-in-python/
        ...
        for i in range(3):  # for nxn replace 3 with n
            a = []
            for j in range(3):  # for nxn replace 3 with n
                a.append(int(input()))
            puzzle.append(a)
            # convert to tuple
